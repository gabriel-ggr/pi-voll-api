...
export const atualizarPaciente = async (
  ...
  try {
    ...
    if (senha !== undefined) paciente.senha = encryptPassword(senha);
    if (telefone !== undefined) paciente.telefone = telefone;
    if (possuiPlanoSaude !== undefined) {
      paciente.possuiPlanoSaude = possuiPlanoSaude;
      if (possuiPlanoSaude === true && planosSaude !== undefined) {
        paciente.planosSaude = mapeiaPlano(planosSaude);
      }
    }
    if (estaAtivo !== undefined) paciente.estaAtivo = estaAtivo;
    if (imagem !== undefined) paciente.imagem = imagem;
    if (historico !== undefined) paciente.historico = historico;

    await AppDataSource.manager.save(Paciente, paciente);
    res.status(200).json(paciente);
  } catch (error) {
    res.status(502).json({ message: 'Paciente não foi atualizado!', status: 502 });
  }
};
